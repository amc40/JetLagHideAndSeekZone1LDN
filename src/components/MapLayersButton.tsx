...
// On mobile the header has no room to spare (see mobile UX audit §2.7), so
// this renders as a Drawer opened from the bottom app bar's overflow menu
// instead of a permanently-visible header button. Desktop keeps the header
// button + Popover unchanged.
export const MapLayersButton = ({ className = "" }: { className?: string }) => {
    const isMobile = useIsMobile();
    const $mapLayersDrawerOpen = useStore(mapLayersDrawerOpen);

    if (isMobile) {
        return (
            <Drawer
                open={$mapLayersDrawerOpen}
                onOpenChange={mapLayersDrawerOpen.set}
            >
                <DrawerContent>
                    <DrawerHeader>
                    ...
                    <div className="flex flex-col gap-3 p-4 pb-8 max-h-[65vh] overflow-y-scroll light">
                        <MapLayersSettings />
                    </div>
                </DrawerContent>
            </Drawer>
